...
def optimizer_evidence_checks(
    ...
    *,
    ...
    manifest_errors = optimizer_run_manifest_errors(root, scope_id, expected_context)
...
def optimizer_run_manifest_errors(
    ...
    if artifact_root:
        try:
            if Path(artifact_root).resolve() != root.resolve():
    ...
    _append_hash_path_errors(manifest, "run_manifest_path", "run_manifest_hash", errors)
    run_manifest = _read_json(Path(str(manifest.get("run_manifest_path") or "")))
    if run_manifest:
