...
def _scope_row(
    ...
    *,
    ...
    tests = {
        ...
        "decision_parity": _decision_parity_status(scope, bridges),
...
def _data_reproduction_status(
    ...
    if failed:
        return {
            ...
            "artifact_paths": _report_paths(failed),
    ...
    return {
        ...
        "artifact_paths": _report_paths(reports),
        "full_family_authority": full_family_authority,
    }
...
def _decision_parity_status(
    ...
    failed = [
        bridge for bridge in scope_bridges if bridge["status"] != "formal_decision_parity_passed"
    ]
    if failed:
        if all(_durable_decision_parity_passed(bridge) for bridge in scope_bridges):
            evidence_paths = [
                str(item.get("path", ""))
                for bridge in scope_bridges
                for item in bridge.get("evidence", [])
                if str(item.get("path", "")).endswith("decision_parity_report.json")
            ]
            approval_ready = all(
                bool(bridge.get("approval_ready", False)) for bridge in scope_bridges
            )
            return {
                "result": "pass",
                "runnable": True,
                ...
                "maturity": ",".join(
                    sorted({str(bridge.get("maturity", "")) for bridge in scope_bridges})
                ),
                "eligible_for_optimizer": all(
                    bool(bridge.get("eligible_for_optimizer", False)) for bridge in scope_bridges
                ),
    ...
    approval_ready = all(bool(bridge.get("approval_ready", False)) for bridge in scope_bridges)
    maturity = sorted({str(bridge.get("maturity", "")) for bridge in scope_bridges})
    return {
        "result": result,
        "runnable": True,
        "ran": True,
...
def _durable_decision_parity_passed(bridge: dict[str, Any] | None) -> bool:
    if bridge is None:
        return False
    for item in bridge.get("evidence", []):
...
def _replay_evidence_status(
    ...
    passing = [
        ...
        and (
            ...
            or _historical_walk_forward_leakage_ok(report, test_name)
        )
    ]
    if passing:
        evidence = [
        ...
        if _full_family_authority_blocked(data_reproduction):
            return {
                ...
                "artifact_paths": evidence or _report_paths(passing),
            }
        return {
            "result": "pass",
            "runnable": True,
    ...
    if test_name == "historical_walk_forward":
        failed_leakage = [
            ...
            for report in scope_reports
            if (report.get("tests") or {}).get(test_name, {}).get("ok") is True
            and not _historical_walk_forward_leakage_ok(report, test_name)
        ]
        if failed_leakage:
            status = _blocked_status(
            ...
            status["artifact_paths"] = [
                path
                for report in failed_leakage
                for path in (report.get("tests") or {}).get(test_name, {}).get(
                    "artifact_paths",
                    [],
                )
                if path
            ] or _report_paths(failed_leakage)
            return status
    ...
    if scope_reports:
        ...
        status["artifact_paths"] = artifact_paths or _report_paths(scope_reports)
        return status
    return _blocked_status(missing_reason, missing_details)


def _historical_walk_forward_leakage_ok(report: dict[str, Any], test_name: str) -> bool:
    artifact_paths = [
        path
        for path in (report.get("tests") or {}).get(test_name, {}).get("artifact_paths", [])
        if path
...
def _full_family_authority_status(
    ...
    data_repo_root = agent_root / "trading_assistant_data"
    requirement_path = Path(scope.full_family_requirement_path)
    if not requirement_path.is_absolute():
        requirement_path = agent_root / requirement_path
...
def _report_paths(reports: list[dict[str, Any]]) -> list[str]:
    return [str(report.get("report_path") or "") for report in reports if report.get("report_path")]
...
def _default_artifact_root() -> Path:
    return _repo_root() / "artifacts" / "validation" / "validation_matrix"
