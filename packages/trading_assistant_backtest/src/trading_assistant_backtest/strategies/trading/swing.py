"""Swing-family bridge backed by the production ``trading`` live-shadow harness."""
...
DECISION_API_VERSION = "trading_swing_live_shadow_decision_api_v1"


def build_trading_swing_decision_parity_report(
    ...
    *,
    ...
    """Build a parity report from the trading repo's swing live-shadow fixtures."""
    ...
    with tempfile.TemporaryDirectory(prefix="trading-swing-parity-") as scratch:
...
def _decision_traces_from_fixture(
    ...
    if surface == "TPC":
        contract = asyncio.run(run_layer2_contract("TPC", path))
        return (
            _trace_events(contract.live, fixture, surface="swing:TPC"),
            _trace_events(contract.replay, fixture, surface="swing:TPC"),
        )
    if surface == "swing_family":
        family = asyncio.run(run_layer3_family_contract("swing", path))
        live_events: list[DecisionTraceEvent] = []
        adapter_events: list[DecisionTraceEvent] = []
        ...
        live_events.append(_family_event(family.live_family_state, fixture, surface="swing:family"))
        adapter_events.append(
            _family_event(family.replay_family_state, fixture, surface="swing:family")
        )
        return live_events, adapter_events
    raise ValueError(f"unsupported trading swing parity surface: {surface!r}")
