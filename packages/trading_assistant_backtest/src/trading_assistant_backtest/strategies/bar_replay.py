"""Shared bar-replay evaluator primitives for shadow-validated non-crypto scopes."""
...
class BarReplayPluginBase:
    ...
    def __init__(self, plugin_id: str, strategy_id: str) -> None:
        ...
        self.supported_symbols = list(self.CONFIG.supported_symbols)
        self.supported_timeframes = list(self.CONFIG.supported_timeframes)
        self.replay_engine_version = self.CONFIG.replay_engine_version
        self._baseline: BarReplayBaseline | None = None
        self._incumbent: ReplayResult | None = None

    def load_baseline(
        self, manifest: MonthlyRunManifest, data_bundle: DataBundleManifest
    ) -> BarReplayBaseline:
        baseline = BarReplayBaseline(
            manifest=manifest,
    ...
    def run_incumbent(self, window: WindowSpec, baseline: Any) -> ReplayResult:
        ...
        result = run_bar_replay(
            ...
            window=window,
            config=replay_baseline.config,
            threshold_bps=replay_baseline.config.threshold_bps,
            position_weight=replay_baseline.config.position_weight,
            max_positions=replay_baseline.config.max_positions,
    ...
    def run_diagnostics(self, replay: ReplayResult) -> dict[str, Any]:
        return {
            "schema_version": self.CONFIG.diagnostics_schema_version,
            "replay_engine_version": self.CONFIG.replay_engine_version,
    ...
    def evaluate_candidate(self, candidate: Candidate, window: WindowSpec) -> CandidateEvaluation:
        ...
        params = _candidate_params(candidate, self.CONFIG)
        patch_payload = evaluated_patch_payload(
            candidate,
            params,
            scope_family=self.CONFIG.family,
        )
        result = run_bar_replay(
        ...
        adoption_enabled = self._baseline.adoption_enabled or self.CONFIG.adoption_enabled
...
def run_bar_replay(
    *,
    ...
    frames = _load_bundle_frames(data_bundle, bundle_manifest_path, config)
    ...
    for key, frame in sorted(frames.items()):
        ...
        if len(item) < 2 or len(trades) >= max_positions:
            continue
    ...
    return ReplayResult(
        ...
        diagnostics={
            ...
            "replay_engine_version": config.replay_engine_version,
            "candidate_id": candidate_id,
            "bundle_id": data_bundle.bundle_id,
            "bundle_checksum": data_bundle.bundle_checksum,
            ...
            "coverage": coverage,
            "lineage": _bundle_lineage(data_bundle, bundle_manifest_path),
            "threshold_bps": threshold_bps,
            "position_weight": position_weight,
...
def _as_baseline(value: Any) -> BarReplayBaseline:
    if not isinstance(value, BarReplayBaseline):
        raise TypeError("bar replay baseline is unavailable")
    return value


def _candidate_params(candidate: Candidate, config: BarReplayConfig) -> dict[str, float | int]:
    patch = effective_parameter_patch(candidate, scope_family=config.family)
    ...
    max_positions += patch_int(patch, "max_positions_delta")
    return {
...
def _load_bundle_frames(
    ...
    for item in data_bundle.slice_manifests:
        if item.market != config.market or item.source != config.source:
            continue
        if item.symbol not in config.supported_symbols:
            continue
        if item.timeframe not in config.supported_timeframes:
            continue
        ...
        rows.append(
            {
                ...
                "source_endpoint": lineage.get("source_endpoint", ""),
                "export_id": lineage.get("export_id", ""),
                "config_hash": lineage.get("config_hash", ""),
                "session_policy": lineage.get("session_policy", ""),
                "authority_contract_id": lineage.get("authority_contract_id", ""),
